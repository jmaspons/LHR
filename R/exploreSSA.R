...
exploreSSA<- function(x0L, params, transitionMat, rateFunc, maxTf=10, replicates=100,
  ...
  resStats<- as.data.frame(matrix(NA_real_, nrow=length(x0L) * nrow(params), ncol=15,
  ...
  for (i in 1:nrow(params)){
    ...
    for (j in seq_along(x0L)){
      ...
      tmp<- data.frame(idScenario=rownames(params)[i], N0=sum(x0L[[j]]), summary(popDtF), stringsAsFactors=FALSE) ## TODO check params column. It's always 1!!
