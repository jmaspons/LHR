...
envir<- data.frame(var=seq(0, 0.15, length=4))
envir<- data.frame(var=c(0, 0.045, 0.1, 0.15))
...
## regular sampling
strategy<- CreaEstrategiesLambdaLliure(dimLength=15, constrains=TRUE, separa_B_sJ=TRUE, roundB=TRUE)
...
## shape1 < 1 take many time qbetanbinom(limit, shape1, shape2, lower.tail=TRUE)
# betaPar<- fbeta(simulation$sA, simulation$varA)
# x<- betaPar$shape1 < 1
# p1000<- pbetanbinom(1000, simulation$n0,  betaPar[[1]], betaPar[[2]])
# p15000<- pbetanbinom(15000, simulation$n0,  betaPar[[1]], betaPar[[2]])
...
# 
# library(rgl)
# plot3d(simulation$sA, simulation$varA, p15000, col=color)
# 
# 
...
# 
# 
# sA<-seq(.28, .95, length=15)
# varSa<- seq(0, .20, length=100)
# s<- expand.grid(mean=sA, var=varSa)
# betaPar<- fbeta(s$mean, s$var)
# p15000par<- pbetanbinom(15000, 1, betaPar$shape1, betaPar$shape2)
# s<- data.frame(s, betaPar, p15000par)
# x<- which(p15000par >= 0.95)
# plot(s)
# points(s[-x,], col="red", pch=16)
# points(s[is.na(betaPar[[1]]),], pch=16)
# abline(h=.15)
# abline(h=.1)
# abline(h=.045)
# levelplot(p15000par ~ s$mean * s$var)
# 
# s[x,]
# betaPar<- betaPar[order(-betaPar[[1]], -betaPar[[2]]),]
# color<- topo.colors(nrow(betaPar))
# 
# plot(expand.grid(c(1, 100), c(0, 1)), type="n")
# 
# for (i in 1:nrow(betaPar)){
#   if (is.finite(betaPar[[1]][i])){
#     if (x[i]) color<- "red" else color<- 1
#     points(0:100, (exp(dbetanbinom(0:100, 27, betaPar[[1]][i], betaPar[[2]][i], log=TRUE))), col=color[i], type="s")
#   }
